...
function LoginForm() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!res.ok) {
        setErrors({ general: res.error === "Invalid credentials" ? "Identifiants invalides." : res.error });
        return;
      }
      ...
      setErrors({
        general: "Une erreur wtf viens de se produire, force",
      });
    } finally {
      setIsLoading(false);
    }
    ...
    <main className="min-h-screen bg-bg text-text-primary flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-[380px] space-y-4">
        ...
        <section className="bg-surface border border-border rounded-xl p-4 text-center text-xs text-text-secondary">
          ...
          <Link to="/register" className="text-btn-primary font-semibold hover:underline">
            S'inscrire
          </Link>
        </section>
      </div>
